    ...
    public partial class BorrarPrograma : Form
    {
        ...
        private void ListaMunicipios_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ListaMunicipios.SelectedItem != null)
            {
                ...
                var filter = Builders<Programa>.Filter.Eq("Nombre_Programa", textCodigo.Text);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            // Verificar si existen subsidios con el mismo nombre de programa
            var subsidiosConPrograma = miDB.GetCollection<Subsidio>(configDB.SubsidiosCollectionName)
                                            .Find(s => s.Programa == textCodigo.Text).Any();

            if (subsidiosConPrograma)
            {
                MessageBox.Show("No se puede eliminar el programa porque tiene subsidios asociados.");
            }
            else
            {
                var filtro = Builders<Programa>.Filter.Eq(m => m.Nombre_Programa, textCodigo.Text);
                var resultado = miDB.GetCollection<Programa>(coleccionProgramas).DeleteOne(filtro);

                if (resultado.DeletedCount == 1)
                {
                    MessageBox.Show("El programa fue eliminado correctamente.");
                    this.Close();
                }
                else
                {
